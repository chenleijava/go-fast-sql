...
func TestDB_Close(t *testing.T) {
	...
	for j <= 1 {
		dbh.BatchInsert("INSERT INTO dsp_db_setting.app_info (app_name, app_sigin, app_packagename, app_size, app_status, app_version_name, app_version_number, app_update_role_name,app_update_time) values (?, ?, ?, ?, ?, ?, ?, ?, ?)", "跳跳微信1111:"+strconv.Itoa(j), "xxas", "com.xx.google.co", 12, 0, "1.0", 1, "石头哥", time.Now().UTC())
		j++
	}
	dbh.FlushAll()
